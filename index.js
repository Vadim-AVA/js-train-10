...
function customAssign(...objects) {
  ...
  const returnedTarget = Object.assign(...objects);
  for (let value in returnedTarget) {
    if (typeof returnedTarget[value] == "number") {
      returnedTarget[value] = String(returnedTarget[value]);
    }
  }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  Object.freeze(returnedObj);
  if (Object.isFrozen(returnedObj)) {
    return returnedObj;
  }
  else {
    return false;
...
function filterObjectsByKey(arr, key) {
  ...
  // const arrByID = arr.filter(key);
  // return arr.filter((el) => el[1] == key);
  // return numbers.indexOf(number) !== index
  const filteredObj = arr.filter((obj) => obj.hasOwnProperty(key));
  return filteredObj;
}
